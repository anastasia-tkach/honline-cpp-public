...
//HModel with wrist
const uint num_blocks = uint(31);
const uint num_centers = uint(39);
...
uniform int use_indicator_texture;
	...
	for (uint j = uint(0); j < num_blocks; j++) {
        ...
        if (block[2] < RAND_MAX) {
            ...
            if (length(p - i) < min_distance) {				
				...
				min_normal = normal;
				min_b = j;
				if (use_indicator_texture == 0) break; // comment this for recording synthetic data
            }
        }
        if (block[2] >= RAND_MAX) {						
...
void main() {	
	if (use_indicator_texture == 1 && texture(silhouette, uv).r == 0) {
		out_depth = uint(32767);
		out_color = uint(255);
		return;
	}
