...
public:
	static void outer_product_J(const thrust::device_vector<float>& J, ///< input
                              thrust::device_vector<float>& JtJ, ///< output
                              int num_constr, ///< #rows of J
                              int num_pars) ///< #cols of J
    {
		assert(J.size() == (num_constr * num_pars)); ///< check jacobian rows
        assert(JtJ.size()==(num_pars * num_pars));
        outer_product((float*)thrust::raw_pointer_cast(&J[0]),
...
public:
	static void vector_product_J(const thrust::device_vector<float>& J, ///< input
                               ...
                               int num_pars) ///< #cols of J
    {
        ...
        vector_product((float*)thrust::raw_pointer_cast(&J[0]),
...
private:
    ...
    static void outer_product(const float* J, float* JtJ, int num_constr, int num_parameters /*num_thetas*/){
...
#define ROW_MAJOR_INPUT
        ...
        cublasStatus_t status = cublasSgemm(CublasHelper::handle(), 
											...
											num_parameters, num_parameters, num_constr,
                                            &alpha,
                                            J, // A 
											num_parameters, // ld_A
                                            J, // B, 
