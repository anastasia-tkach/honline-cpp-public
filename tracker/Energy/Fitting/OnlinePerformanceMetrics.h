	...
	void find_rendered_pixels_outside_sensor_silhouette(vector<int> & rendered_pixels, const cv::Mat & rendered_model, const cv::Mat & sensor_silhouette) {
		...
		for (int row = 0; row < sensor_silhouette.rows; ++row) {
			for (int col = 0; col < sensor_silhouette.cols; ++col) {
				if ((rendered_model.at<unsigned short>(row, col) < BACKGROUND && rendered_model.at<unsigned short>(row, col) != 0 ) && sensor_silhouette.at<uchar>(row, col) != 255) {
...
public:
	...
	float compute_rastorized_2D_metric(const cv::Mat & rendered_model, const cv::Mat & sensor_silhouette, const cv::Mat & sensor_silhouette_distance_transform) {
		...
		for (size_t index = 0; index < rendered_pixels.size(); index++) {
			...
			int closest_index = sensor_silhouette_distance_transform.at<int>(sensor_silhouette.rows - offset_y - 1, offset_x); // or in reverse;			
			int row = closest_index / sensor_silhouette.cols;
			int col = closest_index - sensor_silhouette.cols * row;
	...
	void write_rendered_model(const cv::Mat & rendered_model, const cv::Mat & rendered_silhouette, size_t current_frame, string path) {
		unsigned short mval = std::numeric_limits<unsigned short>::max();
		cv::Mat normalized_rendered_model;
		cv::normalize(rendered_model, normalized_rendered_model, 0, mval, cv::NORM_MINMAX);
		cv::namedWindow("rendered_model", cv::WINDOW_AUTOSIZE);
		cv::imshow("rendered_model", normalized_rendered_model); cv::waitKey(3);
