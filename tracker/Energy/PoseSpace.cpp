...
namespace energy {
	...
	static void on_mouse_pca(int event, int pixels_x, int pixels_y, int f, void* ptr) {
		...
		_worker->model->update_parameters(theta_std);
	...
	void PoseSpace::track(LinearSystem & system, std::vector<float> _theta) {
		if (!(settings->enable_joint_pca || settings->enable_split_pca))
			return;
		Eigen::Map<Eigen::Matrix<Scalar, num_thetas, 1>> theta(_theta.data());
		...
		LinearSystem system_pca(worker->model->num_parameters + m);
		system_pca.lhs.block(0, 0, worker->model->num_parameters, worker->model->num_parameters) = system.lhs;
		system_pca.rhs.segment(0, worker->model->num_parameters) = system.rhs;
		system = system_pca;

		VectorN y = theta.segment(num_thetas_ignore, n); y = y - mu;
		...
		if (settings->enable_joint_pca) {
			...
			//system.lhs.block(num_thetas_ignore, num_thetas_ignore, n + m, n + m) += weight_fingers * LHS;
			//system.rhs.segment(num_thetas_ignore, n + m) += weight_fingers * rhs;

			system.lhs.block(num_thetas_ignore, num_thetas_ignore, n, n) += weight_fingers * LHS.block(0, 0, n, n);
			system.lhs.block(worker->model->num_parameters, num_thetas_ignore, m, n) += weight_fingers * LHS.block(n, 0, m, n);
			system.lhs.block(num_thetas_ignore, worker->model->num_parameters, n, m) += weight_fingers * LHS.block(0, n, n, m);
			system.lhs.block(worker->model->num_parameters, worker->model->num_parameters, m, m) += weight_fingers * LHS.block(n, n, m, m);

			system.rhs.segment(num_thetas_ignore, n) += weight_fingers *  rhs.segment(0, n);
			system.rhs.segment(worker->model->num_parameters, m) += weight_fingers *  rhs.segment(n, m);
		}
		...
		if (settings->enable_split_pca) {
			...
			fingers_bending_latent_variable = x4[0];
			...
			//system.lhs.block(num_thetas_ignore, num_thetas_ignore, n + m, n + m) += weight_thumb * LHS1;
			//system.rhs.segment(num_thetas_ignore, n + m) += weight_thumb *  rhs1;

			//system.lhs.block(num_thetas_ignore, num_thetas_ignore, n + m, n + m) += weight_fingers * LHS4;
			//system.rhs.segment(num_thetas_ignore, n + m) += weight_fingers * rhs4;

			system.lhs.block(num_thetas_ignore, num_thetas_ignore, n, n) += weight_thumb * LHS1.block(0, 0, n, n) + weight_fingers * LHS4.block(0, 0, n, n);
			system.lhs.block(worker->model->num_parameters, num_thetas_ignore, m, n) += weight_thumb * LHS1.block(n, 0, m, n) + weight_fingers * LHS4.block(n, 0, m, n);
			system.lhs.block(num_thetas_ignore, worker->model->num_parameters, n, m) += weight_thumb * LHS1.block(0, n, n, m) + weight_fingers * LHS4.block(0, n, n, m);
			system.lhs.block(worker->model->num_parameters, worker->model->num_parameters, m, m) += weight_thumb * LHS1.block(n, n, m, m) + weight_fingers * LHS4.block(n, n, m, m);

			system.rhs.segment(num_thetas_ignore, n) += weight_thumb *  rhs1.segment(0, n) + weight_fingers *  rhs4.segment(0, n);
			system.rhs.segment(worker->model->num_parameters, m) += weight_thumb *  rhs1.segment(n, m) + weight_fingers *  rhs4.segment(n, m);
		}
	}
