...
namespace energy {
	...
	void Collision::collision_compute_update(LinearSystem &system, Scalar omega) {
		...
		const int k = min(s.size(), t.size());
		Scalar c = 2;

		Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic> J = Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic>::Zero(c * k, model->num_parameters);
		Eigen::Matrix<Scalar, Eigen::Dynamic, 1>  F = Eigen::Matrix<Scalar, Eigen::Dynamic, 1>::Ones(c * k, 1);
		
		Scalar fraction = settings->collision_fraction;
		Matrix_3xN jacobian;
		for (int i = 0; i < k; ++i) {	
			if (energy::Energy::vector_has_nan(n[i])) cout << "nan or inf in collision normal" << endl;
			if (energy::Energy::vector_has_nan(s[i].p)) cout << "nan or inf in collision source" << endl;
			if (energy::Energy::vector_has_nan(t[i].p)) cout << "nan or in collision target" << endl;

			jacobian = model->jacobian_theta(s[i].p, s[i].id);
			if (energy::Energy::matrix_has_nan(jacobian)) cout << "nan in collision jacobian" << endl;
			if (energy::Energy::vector_has_nan(n[i].transpose() * jacobian)) cout << "nan in collision n' * jacobian" << endl;
			J.block(c * i, 0, 1, num_thetas) = n[i].transpose() * jacobian;
			F.block(c * i, 0, 1, 1) = fraction * n[i].transpose() * (t[i].p - s[i].p);

			jacobian = model->jacobian_theta(t[i].p, t[i].id);
			if (energy::Energy::matrix_has_nan(jacobian)) cout << "nan in collision jacobian" << endl;
			if (energy::Energy::vector_has_nan(n[i].transpose() * jacobian)) cout << "nan in collision n' * jacobian" << endl;
			J.block(c * i + 1, 0, 1, num_thetas) = n[i].transpose() * jacobian;
			F.block(c * i + 1, 0, 1, 1) = fraction *  n[i].transpose() * (s[i].p - t[i].p);
		}
		J.block(0, 0, c * k, num_thetas_rigid_motion) = Matrix_MxN::Zero(c * k, num_thetas_rigid_motion);

		if (system.has_nan()) cout << "of the before Collisions term" << endl;
		if (energy::Energy::matrix_has_nan(J)) {
			cout << "Collisions jacobian has nan" << endl;
		}
		if (energy::Energy::vector_has_nan(F)) cout << "Collisions function has nan" << endl;

		system.lhs += omega * J.transpose() * J;
		system.rhs += omega * J.transpose() * F;

		if (system.has_nan()) cout << "of the Collisions term" << endl;
	}
