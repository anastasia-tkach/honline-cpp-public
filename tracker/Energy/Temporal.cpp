...
namespace energy {
	...
	void extract_positions(Model * model, const vector<int> & joint_ids, const vector<int> & center_ids,
		...
		model->update_parameters(theta_previous);
		...
		model->update_parameters(theta_current);
		model->update_centers();
	}
	...
	void Temporal::track(LinearSystem& system, int fid, bool first_order) {
		...
		{
			Eigen::Matrix<Scalar, d * num_temporal, Eigen::Dynamic> J = Eigen::Matrix<Scalar, d * num_temporal, Eigen::Dynamic>::Zero(d * num_temporal, model->num_parameters);
			...
			for (size_t i = 0; i < joint_ids.size(); ++i) {
				...
				J.block(3 * i, 0, 3, num_thetas) = model->jacobian_theta(pos_frame, phalange_ids[i]);	
			...
			Eigen::Matrix<Scalar, Eigen::Dynamic, d * num_temporal> JT = J.transpose();
			system.lhs += omega * JT * J;
			system.rhs += omega * JT * e;
		}

		///--- Check
		if (system.has_nan()) cout << "of the Temporal term" << endl;
	}
