...
void DataStream::save_as_image(std::string path, size_t frame_index) {
	std::ostringstream stringstream;
	stringstream << std::setw(7) << std::setfill('0') << frame_index;
	std::string filename;
	filename = path + "depth-" + stringstream.str() + ".png";
	cv::imwrite(filename, frames.at(frame_index)->depth);
	filename = path + "color-" + stringstream.str() + ".png";
	cv::imwrite(filename, frames.at(frame_index)->color);
	if (frames.at(frame_index)->full_color.data) {
		filename = path + "full_color-" + stringstream.str() + ".png";
		cv::imwrite(filename, frames.at(frame_index)->full_color);
	}
	//filename = path + "mask-" + stringstream.str() + ".png";
	//cv::imwrite(filename, frames.at(frame_index)->silhouette);	
	cout << filename << endl;
