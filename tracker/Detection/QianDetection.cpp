...
QianDetection::QianDetection(Worker *worker) : worker(worker) {
    ...
    find_fingers = new FindFingers(detection, worker->camera, worker->handfinder, &worker->current_frame);
	this->num_parameters = worker->model->num_parameters;
...
void QianDetection::detect_compute_points_update(LinearSystem& system_detection, size_t permutation, bool verbose) {
	...
	for (int i = 0; i < k; ++i) {
		...
		js = worker->model->jacobian_theta(s, worker->model->jointid_to_phalangeid_map[id]);
	...
	system_detection.lhs.noalias() += 3 * J.transpose() * J;
...
void QianDetection::detect_compute_lines_update(LinearSystem& system_detection, size_t permutation, bool verbose) {
	...
	system_detection.lhs.noalias() += J.transpose() * J;
...
void QianDetection::my_damping_track(LinearSystem& system) {
	...
	for (int i = 0; i < num_thetas; ++i) {
		if (worker->model->pose_dofs[i].type == TRANSLATION_AXIS) d(i) = lambda_translation;
	...
	for (int i = 0; i<system.lhs.rows(); i++) {
		for (int j = 0; j<system.lhs.cols(); j++) {
			if (isnan(system.lhs(i, j))) {			
				std::cout << "nan detected in the lhs" << std::endl;
				return true;
			}
			if (isinf(system.lhs(i, j))) {
				std::cout << "inf detected in the lhs" << std::endl;
				return true;
			}
		}
		if (isnan(system.rhs(i))) {		
			std::cout << "nan detected in the rhs" << std::endl;
			return true;
		}
		if (isinf(system.rhs(i))) {
			std::cout << "inf detected in the rhs" << std::endl;
			return true;
		}
	}
	return false;
...
void QianDetection::optimize_current_permutation(size_t permutation, size_t direction, const Eigen::Matrix<Scalar, num_thetas, 1> & theta_0, bool verbose, std::vector<float> & theta_std) {
	...
	for (size_t optimization_iteration = 0; optimization_iteration < num_iterations; optimization_iteration++) {
		...
		worker->model->update_parameters(theta_std);
		worker->model->update_centers();

		if (optimization_iteration == num_iterations - 1) break;

		LinearSystem system_detection(num_parameters);
		...
		LinearSystem system_detection_resized(num_parameters);
		system_detection_resized.lhs.block(0, 0, num_thetas, num_thetas) = system_detection.lhs;
		system_detection_resized.rhs.segment(0, num_thetas) = system_detection.rhs;
		system_detection = system_detection_resized;

		worker->E_limits.track(system_detection, theta_std, worker->model->get_beta());
		...
		//	if (isnan(delta_theta(i))) { has_nan = true; break; }
		//} if (has_nan) std::cout << "nan detected in update" << std::endl << std::endl;
...
void QianDetection::detection_iterate() {
	...
	worker->model->update_parameters(theta_min_energy);
	worker->model->update_centers();
}
