...
FindFingers::FindFingers(DetectionStream *detection, Camera * camera, HandFinder * handfinder, DataFrame * current_frame) : detection(detection), camera(camera), handfinder(handfinder), current_frame(current_frame) {}
...
Matrix_MxN FindFingers::crop_image(const Matrix_MxN &depth, size_t & min_row, size_t & max_row, size_t & min_column, size_t & max_column) {
...
void FindFingers::restore_original_image(size_t min_row, size_t max_row, size_t min_column, size_t max_column, 
	std::vector<Vector2> & xy_finger_tips, std::vector<Vector2> & z_finger_tips,
	std::vector<Vector2> & xy_finger_roots, std::vector<Vector2> & z_finger_roots, Vector3 & palm_center) {
...
void FindFingers::compute_finger_pixels_svd(const Matrix_MxN & finger, Matrix_Nx2& data, Eigen::JacobiSVD<Matrix_2x2> & svd,
                                            float & mean_rows, float & mean_columns, Eigen::Matrix<Scalar, 2, 2> & covariance) {
    ...
    covariance = data.transpose() * data / n;
...
bool FindFingers::verify_finger_shape(const Matrix_MxN &finger, Vector2s v, FindFingers::Mode mode) {
    ...
    compute_finger_pixels_svd(finger, data, svd, mean_rows, mean_columns, covariance);
...
void FindFingers::find_finger_directions(FindFingers::Mode mode) {
    ...
    for (size_t i = 0; i < num_detected_fingers; i++) {
        ...
        VectorN v;
        if (mode == XY) {
            compute_finger_pixels_svd(xy_finger_segments[i], data, svd, mean_rows, mean_columns, covariance);
            v = xy_finger_tips[i];
        }
        if (mode == Z) {
            compute_finger_pixels_svd(z_finger_segments[i], data, svd, mean_rows, mean_columns, covariance);
            v = z_finger_tips[i];
        }
        VectorN s = svd.singularValues().array().pow(0.5);
        Matrix_MxN U = svd.matrixU();
...
void FindFingers::feed_results(const Matrix_MxN &depth) {
    ...
    p1 = camera->depth_to_world(palm_center(1), palm_center(0), palm_center(2));
    p2 = camera->depth_to_world(palm_center(1) + palm_normal(1), palm_center(0) + palm_normal(0), palm_center(2) + palm_normal(2));
    ...
    for (size_t i = 0; i < xy_finger_tips.size(); i++) {
        ...
        p1 = camera->depth_to_world(xy_finger_tips[i](1), xy_finger_tips[i](0), d1);
                p2 = camera->depth_to_world(xy_finger_roots[i](1), xy_finger_roots[i](0), d2);
    ...
    for (size_t i = 0; i < z_finger_tips.size(); i++) {
        ...
        p1 = camera->depth_to_world(z_finger_tips[i](1), z_finger_tips[i](0), d1);
                p2 = camera->depth_to_world(z_finger_roots[i](1), z_finger_roots[i](0), d2);
...
Matrix_MxN FindFingers::process_input() {
    ...
    cv::Mat mask = handfinder->sensor_silhouette.clone();    
    ...
    cv::Mat cv_depth = current_frame->depth.clone();
...
void FindFingers::find_thumb(const Matrix_MxN &depth) {
    ...
    // Find hand side
    wristband_center = camera->world_to_image(handfinder->wristband_center());
    Vector2 wristband_shift = camera->world_to_image(handfinder->wristband_center() + handfinder->wristband_direction());
...
bool FindFingers::find_fingers_main(bool display) {
    ...
    Matrix_MxN depth = crop_image(original_depth, min_row, max_row, min_column, max_column);
    ...
    restore_original_image(min_row, max_row, min_column, max_column, xy_finger_tips, z_finger_tips, xy_finger_roots, z_finger_roots, palm_center);
