...
public:
	...
	int num_frames_since_calibrated = -1;
private:
	vector<float> center_index_to_length_certainty_map;
	vector<float> center_index_to_x_certainty_map;
	vector<float> center_index_to_y_certainty_map;
	vector<float> center_index_to_radius_certainty_map;
...
public:
	enum SHADERMODE { NORMAL, FRAMEBUFFER };
	...
	ConvolutionRenderer(Model *model, bool real_color, std::string data_path, int camera_heigth, int camera_width);

	ConvolutionRenderer(Model *model, ConvolutionRenderer::SHADERMODE mode, const Eigen::Matrix4f& view_projection, std::string data_path, int camera_heigth, int camera_width);

	void send_vertices_to_shader(std::string vertices_name);

	void setup_canvas();

	void pass_model_to_shader(bool fingers_only, bool use_indicator_texure, bool wristband_not_found);

	void pass_beta_certainty_to_shader();
	...
	void render(bool wristband_not_found);

	void render_offscreen(bool fingers_only, bool use_indicator_texure);
